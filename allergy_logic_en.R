output$pollGraph <- renderUI({
        ...
        # !!! filter mood values by timespan
        if(year == 0){
                ...
                if(nrow(poll) > 0){
                        ...
                        p <- p + scale_y_continuous(
                                sec.axis = sec_axis(~(.+1)*6/5,
                                                    name = "< bad - Mood - good >",
                                                    breaks = 1:6))
                        p <- p + labs(y = "< weak - Pollination - strong >",
                                      x = "",
                                      colour = "")
                        switch(time_select,
        ...
        if(poll_info && mood_info){
                tagList(
                        tags$p("Please note: there is no pollination and mood data available for the selected time period", 
                               style="text-align: center;"))
        } else {
                if(poll_info){
                        tags$p("Please note: there is no pollination data available for the selected time period", 
                               style="text-align: center;")
                } else {
                        if(mood_info){
                                tagList(tags$p("Please note: there is no mood data available for the selected time period", 
                                               style="text-align: center;"),
                                        renderPlot({ p }))
                        } else {
